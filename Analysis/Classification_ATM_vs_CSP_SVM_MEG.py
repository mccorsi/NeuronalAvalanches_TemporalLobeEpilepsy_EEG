...
for subject in subjects:
    ...
    for f in freqbands:
        ...
        info = create_info([f'CSP{i}' for i in range(kk_components)], sfreq=ep_meg.info['sfreq'], ch_types='eeg')
        ep_csp_m = EpochsArray(epochs_data_csp_m, info, ep_meg.events, event_id=ep_meg.event_id)
...
for f in freqbands:
    ...
    for kk_val_duration in opt_val_duration:
        for kk_zthresh in opt_zthresh:
            ...
            filename = path_csv_root + "/SVM/IndivOpt_ComparisonDiff_SVM_Classification-allnode-2class-right_hand-rest-" + "n_csp_cmp-" + str(kk_components) + "zthresh-" + str(kk_zthresh) + "-freq-" + str(fmin) + '-' + str(fmax) + '-nbSplit' + str(nbSplit) + "_val_duration_" + str(kk_val_duration)
    ...
    for f in freqbands:
        ...
        for kk_val_duration in opt_val_duration:
            filename = path_csv_root + "/SVM/IndivOpt_ComparisonDiff_SVM_Classification-allnode-2class-right_hand-rest-" + "n_csp_cmp-" + str(
                ...
                nbSplit) + "_val_duration_" + str(kk_val_duration)
...
#%% Load results for statistical tests
opt_zthresh = [1.4, 1.6, 1.8, 2, 2.2, 2.4, 2.6, 2.8, 3] 
opt_val_duration = [2, 3, 4, 5, 6, 7, 8]
kk_components = 8
freqbands= {'paper': [3, 40]}
...
for subj in tqdm(subjects, desc="subject"):
    ...
    for i_zth, kk_zthresh in enumerate(opt_zthresh):
        for i_val, kk_val_duration in enumerate(opt_val_duration):
            ...
            temp_stat = pd.read_csv(
                ...
                "-freq-" + str(fmin) + '-' + str(fmax) + '-nbSplit' + str(nbSplit) +
                "_val_duration_" + str(kk_val_duration) + ".csv") # 1 file for all subjects here
    ...
    temp_res_opt = pd.read_csv(
        ...
        "-freq-" + str(fmin) + '-' + str(fmax) + '-nbSplit' + str(nbSplit) +
        "_val_duration_" + str(int(optimal_val_duration)) + ".csv"
    )
    ...
    temp_stat_opt = pd.read_csv(
        ...
        "-freq-" + str(fmin) + '-' + str(fmax) + '-nbSplit' + str(nbSplit) +
        "_val_duration_" + str(optimal_val_duration) + ".csv")  # 1 file for all subjects here
...
df_optcfg.to_csv(
    path_csv_root + "/SVM/df_optcfg_SVM_Classification-allnode-2class-right_hand-rest-" + "n_csp_cmp-" + str(
        kk_components) + "-freq-" + str(fmin) + '-' + str(fmax) + '-nbSplit' + str(nbSplit) + ".csv")

df_optcfg_std.to_csv(path_csv_root + "/SVM/df_optcfg_SVM_Classification-allnode-2class-right_hand-rest-" + "n_csp_cmp-" + str(
        kk_components) + "-freq-" + str(fmin) + '-' + str(fmax) + '-nbSplit' + str(nbSplit) + ".csv")
...
g = sns.catplot(x="score",
                ...
                height=6, aspect=2,
                data=res_opt)
plt.savefig(path_figures_root + "IndivOpt_SVM_Classification_"+str(fmin)+'-'+str(fmax)+"NETBCI_GlobalResults_MEG_DK_Sess4-2class_rest_rh_nbSplits"+str(nbSplit)+".pdf", dpi=300)
...
g = sns.catplot(x="score",
                ...
                height=6, aspect=2,
                data=res_opt)
plt.savefig(path_figures_root + "IndivOpt_SVM_Classification_IndivPlot"+str(fmin)+'-'+str(fmax)+"NETBCI_GlobalResults_MEG_DK_Sess4-2class_rest_rh_nbSplits"+str(nbSplit)+".pdf", dpi=300)
...
plt.savefig(path_figures_root + "BestConfig_Study_IndivOpt_SVM_Classification_IndivPlot"+str(fmin)+'-'+str(fmax)+"NETBCI_GlobalResults_MEG_DK_Sess4-2class_rest_rh_nbSplits"+str(nbSplit)+".pdf", dpi=300)
...
g = sns.catplot(x="Avgscore",
                ...
                height=6, aspect=2,
                data=pd_plot_Diff)
plt.savefig(path_figures_root + "SwarmMeanDiff_IndivOpt_SVM_Classification_IndivPlot"+str(fmin)+'-'+str(fmax)+"NETBCI_GlobalResults_MEG_DK_Sess4-2class_rest_rh_nbSplits"+str(nbSplit)+".pdf", dpi=300)
...
g = sns.catplot(x="Avgscore",
                ...
                height=6, aspect=2,
                data=pd_plot_avgsc)
plt.savefig(path_figures_root + "SwarmMeanScores_IndivOpt_SVM_Classification_IndivPlot"+str(fmin)+'-'+str(fmax)+"NETBCI_GlobalResults_MEG_DK_Sess4-2class_rest_rh_nbSplits"+str(nbSplit)+".pdf", dpi=300)
...
plt.savefig(path_figures_root + filename + "_FE.pdf", dpi=300)
...
colors = [[0.5,0.5,0.5], test_magma[4], test_magma[1]]# Set your custom color palette
...
g.ax_marg_x.hist(paired["ATM+SVM"], color=palette_signif[1], alpha=.6)
g.ax_marg_y.hist(paired["CSP+SVM"], color=palette_signif[2], alpha=.6, orientation="horizontal")
...
g.savefig(path_figures_root + filename + "_.png", dpi=300)
...
plt.savefig(path_figures_root + "IndivOpt_SVM_Classification_"+str(fmin)+'-'+str(fmax)+"NETBCI_GlobalResults_MEG_DK_Sess4-2class_rest_rh_nbSplits"+str(nbSplit)+".png", dpi=300)
...
axes.set_ylim(0, 0.1)
plt.savefig(path_figures_root + "IndivOpt_SVM_Classification_STD_"+str(fmin)+'-'+str(fmax)+"NETBCI_GlobalResults_MEG_DK_Sess4-2class_rest_rh_nbSplits"+str(nbSplit)+".png", dpi=300)
...
g.ax_marg_x.hist(x_std, color=palette_signif[1], alpha=.6)
g.ax_marg_y.hist(y_std, color=palette_signif[2], alpha=.6, orientation="horizontal")
g.fig.axes[0].spines["bottom"].set_linewidth(6)
g.fig.axes[0].spines["left"].set_linewidth(6)
g.set_axis_labels('ATM+SVM', 'CSP+SVM', fontsize=24, weight='bold')
g.ax_joint.legend_.remove()
...
for xlabel, ylabel in zip(g.ax_joint.get_xticklabels(), g.ax_joint.get_yticklabels()):
    ...
    xlabel.set_weight('bold')
    xlabel.set_rotation(45)
g.savefig(path_figures_root + "IndivOpt_SVM_Classification_STD_"+str(fmin)+'-'+str(fmax)+"NETBCI_GlobalResults_MEG_DK_Sess4-2class_rest_rh_nbSplits"+str(nbSplit)+".png", dpi=300)

#%% compute inter-subject variability
...
#%% plot influence parameters
#list_zthresh=[1.4, 1.6,1.8,2,2.2]
#list_zthresh=[2.4, 2.6, 2.8, 3]
#list_zthresh=[3]
list_zthresh=opt_zthresh
res2plot=res[res["zthresh"].isin(list_zthresh)]
import matplotlib.pyplot as plt
...
axes.legend(fontsize='xx-small', ncol=2, frameon=False, bbox_to_anchor=(0.9,0.8)) #[str(i) for i in list_zthresh],
#axes.legend(bbox_to_anchor=(0.9,0.8))
plt.savefig(path_figures_root + "Rebuttal_InfluenceParameters_all.png", dpi=300)
