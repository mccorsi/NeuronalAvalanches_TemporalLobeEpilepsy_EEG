...
for f in freqbands:
    ...
    for iperm in range(n_perms): # in case we want to do some permutations to increase the statistical power
        ...
        # k_zthresh_edge_nodal = opt_atm_param_nodal[f][0]
        # k_val_duration_edge_nodal = opt_atm_param_nodal[f][1]
        ...
        temp_results_atm_nodal["zthresh"] = [k_zthresh_edge] * len(ppl_atm_nodal)
        temp_results_atm_nodal["val_duration"] = [k_val_duration_edge] * len(ppl_atm_nodal)
