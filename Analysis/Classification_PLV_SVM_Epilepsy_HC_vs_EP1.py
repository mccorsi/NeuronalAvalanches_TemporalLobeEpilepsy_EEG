...
for f in freqbands:
    ...
    for iperm in range(n_perms): # in case we want to do some permutations to increase the statistical power
        ...
        mat_tril_PLV = np.empty((nb_trials, 2278))
        for kk_trial in range(nb_trials):
            temp=mat_PLV[kk_trial,:,:]
            mat_tril_PLV[kk_trial,:] = temp[tril_mat_true]
        PLV_cl = mat_tril_PLV # already flatten

        #PLV_cl = np.reshape(mat_tril_PLV, (nb_trials, nb_ROIs * nb_ROIs))
       # PLV_nodal_cl = np.sum(PLV_cl,1)
    ...
    results_global.to_csv(
            path_csv_root + "/SVM/Tril_HC_EP1_IndivOpt_PLV_SVM_Classification-allnode-2class-" +
...
#%%
results_plv_alone=pd.DataFrame()

for f in freqbands:
    fmin = freqbands[f][0]
    fmax = freqbands[f][1]
    temp_results = pd.read_csv(         path_csv_root + "/SVM/Tril_HC_EP1_IndivOpt_PLV_SVM_Classification-allnode-2class-" +
            "-freq-" + str(fmin) + '-' + str(fmax) + '-nbSplit' + str(nbSplit) + ".csv"
             )

    results_plv_alone = pd.concat((results_plv_alone, temp_results))
...
results_plv_alone2use=temp_results
...
g = sns.catplot(y="test_accuracy",
                ...
                #row="freq",
                height=7, aspect=2,
                data=results_plv_alone2use)
plt.savefig(path_figures_root + "Tril_HC_EP1_IndivOpt_PLV_SVM_Classification_edges_nodes-2class-nbSplits"+str(nbSplit)+"_broad_MEG.pdf", dpi=300)
